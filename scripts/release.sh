...
release-notes --org mattermost --repo cloud-db-factory-vertical-scaling --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch master
...
hub release create -d -F ./build/_output/docs/relnote.md -a ./build/_output/bin/cloud-db-factory-vertical-scaling-linux-arm64 -a ./build/_output/bin/cloud-db-factory-vertical-scaling-linux-amd64 -a ./build/_output/bin/cloud-db-factory-vertical-scaling-darwin-amd64 -a ./build/_output/bin/cloud-db-factory-vertical-scaling-darwin-arm64 ${CIRCLE_TAG}
rm -rf ./build/_output/
